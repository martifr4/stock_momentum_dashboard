...
def test_fear_greed_is_lagged_for_causality(monkeypatch):
    ...
    # Synthetic F&G ramp 0..119 -> normalized; a distinct value per day.
    fng = pd.DataFrame(
    ...
    # social_value at date T must equal the F&G value from T-1 (the lag).
    for i in range(2, 10):
        assert btc["social_value"].iloc[i] == fng["fng_value"].iloc[i - 1]
    # First row has no prior reading -> NaN value, neutral sentiment.
